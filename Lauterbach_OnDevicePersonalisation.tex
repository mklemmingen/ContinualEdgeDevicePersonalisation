...
archived at \href{https://doi.org/10.5281/zenodo.20599082}{DOI:~10.5281/zenodo.20599082}.}}
...
resistance measured as backward-transfer accuracy (BWT)
...
\small
\begin{tabular}{|>{\bfseries}p{2.3cm}|p{3.0cm}|p{2.4cm}|p{3.5cm}|p{4.0cm}|}
\hline
\textbf{Approach} &
...
On-chip updates (RPi3~\cite{b3}; Radxa~Zero~\cite{b13}); pilot (Pi~5): 33.8\,ms, 118\,mJ/update, a $4.0{\times}$ marginal energy advantage over full network training (Section~\ref{sec:challenges}) \\
\hline
...
\caption{Pilot-measurement testbed on Raspberry~Pi~5. The ATORCH
J7-c USB-C inline meter (foreground, 5.13\,V, 1.07\,A) provides a
sanity-check reference; all reported power figures come from the
Pi~5's on-chip Renesas DA9091 PMIC via
\texttt{vcgencmd~pmic\_read\_adc} (see body text for the
rail-exclusion caveat).}
\label{fig:testbed}
\end{figure}
...
of task~0 after training on task~1), confirming the full network
failure mode is real and reproducible on commodity hardware;
quantifying $\mu$-training's resistance needs a real biosignal
...
The Pi~5 is an application processor (Cortex-A76, DRAM,
${\sim}2.8$\,W idle), not a Cortex-M microcontroller (SRAM-only,
$10\text{--}100$\,mW), but both surveyed $\mu$-training works deploy
on Pi-class hardware~\cite{b3,b13}, so we ran the testbed of
Fig.~\ref{fig:testbed} through a trainable-fraction sweep with
on-chip PMIC telemetry (\texttt{vcgencmd pmic\_read\_adc}); this
sums 14 rails but excludes the 5\,V main rail, so peripheral draw
is absent from totals (relative comparisons hold as the offset is
roughly constant). With the
$\mu$-training freeze pattern (2.06\,\% of weights trainable on a
99.7\,k-parameter
ECG-shaped 1D-CNN), each update costs 33.8\,ms and 118\,mJ above
the 2.77\,W Pi~5 idle baseline, versus 137.9\,ms and 475\,mJ for
full network training: a $4.08{\times}$ wall-time and
$4.02{\times}$ marginal energy advantage, stable across the
200-iteration sweep (Fig.~\ref{fig:sweep}). The Pi~5
measurement sits 3--4 orders of magnitude above the
$10^{2}\,\mu$J/update Cortex-M target proposed below; repeating the
sweep on Cortex-M4F/M7 hardware should close this gap.
POET~\cite{POET} and PockEngine~\cite{PockEngine} cut per iteration
cost on such devices via sparse backpropagation. The key
precondition is plausible: the $\mu$-freeze activation footprint
fits the 256\,kB SRAM of a Cortex-M7 such as the
STM32F746G~\cite{PockEngine}.
...
2.77\,W idle baseline (bottom) vs.\ trainable-parameter fraction.
The $\mu$-training point at 2.06\,\% (blue) lies on the flat
regime; cost climbs sharply above ${\sim}3\,\%$ as further
convolutional layers enter the backward pass. Raspberry~Pi~5,
4 threads, batch~32, seq.~length~250, FP32, SGD; peripheral draw
excluded from PMIC totals (see body text).}
\label{fig:sweep}
\end{figure}
...
\caption{PMIC power telemetry over the 440\,s pilot run. Colour
bands: active pipeline phase; grey trace: raw 5\,Hz samples
(14 rails summed via \texttt{vcgencmd pmic\_read\_adc}); black
trace: 2\,s rolling mean; dashed line: 2.77\,W idle reference.
The five training plateaus correspond to the trainable-fraction
sweep; the two rightmost (\textit{half-net}, \textit{full-net})
show visibly longer per-update settle times.}
\label{fig:timeline}
\end{figure*}
...
The surveyed literature shows that $\mu$-training adapts on-device
with $<2\,\%$ of parameters updated and no gradient egress, that
metaplastic BNNs resist forgetting without replay or regularisation
overhead, and that label-free drift detection is a viable
autonomous trigger. None of these sources reports per-update energy
on a named platform, and independent reproductions remain open. The
Pi~5 pilot in Section~\ref{sec:challenges} measures a $4.0{\times}$
marginal energy advantage of $\mu$-training over full network
training, with a confirmed full network forgetting failure
(BWT$=-1.0$) on the same device. The three proposed benchmark
targets are tractable next measurement steps toward cross-study
comparison on Cortex-M-class hardware, where sub-microjoule
per-inference energy is already demonstrated~\cite{b10}.
